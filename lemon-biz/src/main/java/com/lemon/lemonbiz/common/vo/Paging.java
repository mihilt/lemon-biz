...
public class Paging {
	...
	public static String attendPageHtml(int cPage, int numPerPage, int totalContents, String url) {
		StringBuilder pageBar = new StringBuilder();
		...
		int pageBarSize=5;
		//페이지바가 5개이하라면 안보임처리
		if(totalContents/5+1<5) {
		pageBarSize= totalContents/5+1; //페이지바에 나열될 페이지번호의 개수
		}
		//115 / 10 => 12
		int totalPage = (int)Math.ceil((double)totalContents/numPerPage);

		System.out.println("ttototoototo"+totalContents);
		// x * pageBarSize + 1
		// 1 2 3 4 5 -> 1      
		// 6 7 8 9 10 -> 6
		// 11 12 13 14 15 -> 11
